...
\tikzstyle{block} = [draw, fill=blue!20, rectangle, minimum height=3em, minimum width=6em, align=center]
\begin{figure}
\begin{centering}
	{
    \begin{tikzpicture}
    ...
    \draw [->] (5.05,6.2) -- node [midway, above] {$U_1$} (10.95,6.2);
    \draw [<-] (5.05,5.8) -- node [midway, below] {$x$, $\dot{x}$, $y$, $\dot{y}$, $z$, $\dot{z}$} (10.95,5.8);
    \draw [->] (5.05,0.2) -- node [midway, above] {$U_2$, $U_3$, $U_4$} (10.95,0.2);
    \draw [<-] (5.05,-0.2) -- node [midway, below] {$\phi$, $\dot{\phi}$, $\theta$, $\dot{\theta}$, $\psi$, $\dot{\psi}$} (10.95,-0.2);
    
    % To the Observer 
    \draw [->] (6.2,6.2) -- node [midway, left] {} (6.2,3.53);
    \draw [->] (6.6,5.8) -- node [midway, right] {} (6.6,3.53);
    \draw [->] (6.2,-0.2) -- node [midway, left] {} (6.2,2.47);
    \draw [->] (6.6,0.2) -- node [midway, right] {} (6.6,2.47);
    
    
    % Postion Controller to Desired Orientation and Observer
    \draw [-] (4,5.47) -- node [midway, left] {$u_x$, $u_y$} (4,3);
    \draw [->] (4,3) -- node [midway, below] {} (5.95,3);
    \draw [->] (4,3) -- node [midway, below] {} (2.06,3);
    
    % Desired Traj to Desired Orientation
    \draw [->] (1,8.47) -- node [midway, left] {$p_d=[x_d,y_d,z_d,\psi]$} (1,3.67);
    % Desired Traj to Position Controller
    \draw [-] (2.55,9) -- node [midway, above] {} (4,9);
    \draw [->] (4,9) -- node [midway, right] {$p_d$} (4,6.53);
    
    % Desired Orientation Calculator to Attitude COntroller
    \draw [-] (1, 2.33) -- node [midway, below] {} (1,0);
    \draw [->] (1,0) -- node [midway, below] {$\phi_d$, $\theta_d$} (2.94,0);
    
    % Disturbances to the PosCon and AttCon
    \draw [<-] (4.3,5.47) -- node [midway, left] {} (4.3,3.3);
    \draw [-] (4.3,3.3) -- node [midway, above] {$\hat{d}$} (5.95,3.3);
    \draw [<-] (4.3,0.5) -- node [midway, left] {} (4.3,2.7);
    \draw [-] (4.3,2.7) -- node [midway, below] {$\hat{d}$} (5.95,2.7);
	
	% External disturbance arrow 
	 \draw [->] (12, 8.47) -- node [midway, right] {$d$} (12, 6.52);
...
\begin{figure}[H]
\centering
\includegraphics[width=3in,height=4.83in]{Figures/practical/offboard_control_ros.eps}
